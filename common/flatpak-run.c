...
static const struct {const char *env;
  ...
  /* Unset to ensure we don't inherit some weird LD_LIBRARY_PATH. The
     real libs are accessed via the ld.so.cache */
...
static const struct {const char *env;
  ...
  /* We always use LD_LIBRARY_PATH in the flatpak build case, because
     there is no great key for the ld.so.cache cache when building
     random versions. */
                     ...
                     GError        **error)
{
  ...
  add_args (argv_array,
            ...
            "/usr/sbin/ldconfig", "-X", "-C", sandbox_cache_path, NULL);
